...
//  Lists all published SEO articles, each linked to /blog/:slug
// ─────────────────────────────────────────────────────────────────────────────
...
import {
  BookOpen, Clock, ArrowRight, Search, TrendingUp,
  Tag, ChevronRight, Zap, Music, Globe,
} from 'lucide-react';

const CATEGORIES = ['All', 'Music Distribution', 'Spotify Growth', 'TikTok & Social Promotion', 'Publishing & Royalties', 'Music Marketing Agency', 'Playlist Pitching', 'Independent Artist Business'];
...
export function BlogIndex() {
  useSEOPage({
    ...
    keywords: ['music industry blog', 'music distribution guide', 'spotify growth tips', 'music marketing blog', 'independent artist tips', 'playlist pitching guide'],
  ...
  const filtered = posts.filter(p => {
    ...
    const matchQ = !search || p.title.toLowerCase().includes(search.toLowerCase()) || p.targetKeyword?.toLowerCase().includes(search.toLowerCase());
          ...
          <div className="flex gap-2 overflow-x-auto pb-2 scrollbar-none">
            {CATEGORIES.map(cat => (
              <button key={cat} onClick={() => setCategory(cat)}
                className={`flex-shrink-0 px-4 py-2 rounded-xl text-xs font-semibold transition-all border ${category === cat ? 'text-white border-transparent' : 'border-white/[0.07] text-white/50 hover:text-white bg-[#0D0D0D]'}`}
                style={category === cat ? { background: 'linear-gradient(135deg,#7B5FFF,#D63DF6)', borderColor: 'transparent' } : {}}>
                {cat}
              </button>
            ))}
          </div>
    ...
    <Link to={`/blog/${post.slug}`} className="group block">
      <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }}
        ...
        <div className="relative">
          ...
          <h2 className="text-2xl md:text-4xl font-black text-white mb-4 leading-tight group-hover:text-transparent transition-all group-hover:bg-clip-text"
      ...
      <motion.div initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }}
        ...
        <div className="flex items-center justify-between pt-3 border-t border-white/[0.05]">
          <div className="flex items-center gap-3 text-[10px] text-white/30">
            <span className="flex items-center gap-1"><Clock size={10} /> {readTime(post.wordCount)}</span>
          </div>
...
// ── Empty State ───────────────────────────────────────────────────────────────
function EmptyState({ search, category, totalPosts }: { search: string; category: string; totalPosts: number }) {
  if (totalPosts === 0) {
    return (
      <div className="text-center py-32 px-4">
        ...
        <p className="text-white/40 max-w-md mx-auto mb-6 text-sm leading-relaxed">
          Our team is crafting in-depth guides for independent artists and labels. Check back soon, or sign up to get notified when new articles drop.
        </p>
  ...
  return (
    <div className="text-center py-20">
      <p className="text-white/40 text-sm">No articles found for "{search || category}". <button onClick={() => {}} className="text-[#7B5FFF] hover:underline">Clear filters</button></p>
    </div>
  );
}
