// MIXXEA - Blog public pages: BlogIndex (/blog) + BlogPost (/blog/:slug)
import { useState, useEffect } from 'react';
import { Link, useParams } from 'react-router';
import { motion } from 'motion/react';
import { useSEO, useSEOPage } from './seo/useSEO';
import * as api from './dashboard/api';
import {
  BookOpen, Clock, ArrowRight, Search, Tag, ChevronRight,
  Zap, ArrowLeft, Copy, Check, ExternalLink,
} from 'lucide-react';
...
// ─── Shared Nav ───────────────────────────────────────────────────────────────
function BlogNav({ backToBlog }: { backToBlog?: boolean }) {
  return (
    <nav
      ...
      <div className="max-w-7xl mx-auto px-5 py-4 flex items-center justify-between">
        ...
        <div className="flex items-center gap-4">
          {backToBlog ? (
            ...
            <Link to="/blog" className="text-sm font-semibold text-white">Blog</Link>
          )}
          <Link
            to="/auth"
    ...
    <div className="min-h-screen bg-black text-white">
      ...
      <div className="pt-32 pb-16 px-5">
        <div className="max-w-4xl mx-auto text-center">
          <div className="inline-flex items-center gap-2 px-4 py-2 rounded-full border border-[#7B5FFF]/30 bg-[#7B5FFF]/10 mb-6">
            ...
            <span className="text-xs font-semibold text-[#7B5FFF] uppercase tracking-widest">Music Industry Blog</span>
          </div>
          <h1 className="text-4xl sm:text-6xl font-black tracking-tighter mb-4 leading-none">
            Grow Your Music Career
          </h1>
          <p className="text-white/50 text-lg max-w-xl mx-auto mb-10">
            Expert guides on distribution, promotion, playlist pitching, publishing admin, and everything else you need to build a sustainable music business.
          </p>
          ...
          <div className="flex gap-2 overflow-x-auto pb-2">
            {CATEGORIES.map((cat) => (
              <button
                ...
                style={category === cat ? { background: 'linear-gradient(135deg,#7B5FFF,#D63DF6)' } : {}}
              >
                {cat}
              </button>
            ))}
      ...
      <div className="px-5 pb-24 max-w-7xl mx-auto">
        {loading ? (
          ...
          <EmptyState search={search} category={category} total={posts.length} />
        ) : (
          <>
            {!search && category === 'All' && filtered[0] && (
      ...
      <div className="border-t border-white/[0.05] px-5 py-10">
        <div className="max-w-7xl mx-auto flex flex-col sm:flex-row items-center justify-between gap-4">
          ...
          <div className="flex items-center gap-6 text-sm text-white/30">
            ...
            <a href="mailto:onboarding@mixxea.com" className="hover:text-white transition-colors">Contact</a>
          </div>
        </div>
      </div>
    </div>
  ...
  return (
    <Link to={`/blog/${post.slug}`} className="group block">
      <motion.div
        ...
        style={{ background: 'linear-gradient(135deg,rgba(0,196,255,0.06),rgba(123,95,255,0.08),rgba(214,61,246,0.06))' }}
      >
        <div className="absolute inset-0 opacity-0 group-hover:opacity-100 transition-opacity duration-500"
          style={{ background: 'linear-gradient(135deg,rgba(0,196,255,0.04),rgba(123,95,255,0.06),rgba(214,61,246,0.04))' }} />
        <div className="relative">
          <div className="flex items-center gap-3 mb-4">
            <span className="text-[10px] font-black text-[#7B5FFF] bg-[#7B5FFF]/15 px-3 py-1.5 rounded-full uppercase tracking-widest">Featured</span>
            <span className="text-[10px] text-white/30 uppercase tracking-wider">{post.category}</span>
          </div>
          <h2 className="text-2xl md:text-4xl font-black text-white mb-4 leading-tight">{post.title}</h2>
          <p className="text-white/50 text-base mb-6 max-w-2xl leading-relaxed line-clamp-2">{post.metaDescription}</p>
          <div className="flex items-center gap-4 text-xs text-white/30">
            <span className="flex items-center gap-1.5"><Clock size={11} /> {readTime(post.wordCount)}</span>
            <span className="flex items-center gap-1.5"><Tag size={11} /> {post.targetKeyword}</span>
    ...
    <Link to={`/blog/${post.slug}`} className="group block">
      ...
      >
        <div className="flex items-center justify-between mb-4">
          ...
          <span className="text-[10px] text-white/25">{fmtDate(post.publishedAt)}</span>
        </div>
        <h3 className="text-sm font-bold text-white leading-snug mb-3 group-hover:text-[#00C4FF] transition-colors line-clamp-3 flex-1">{post.title}</h3>
        <p className="text-xs text-white/40 leading-relaxed mb-4 line-clamp-2">{post.metaDescription}</p>
        <div className="flex items-center justify-between pt-3 border-t border-white/[0.05]">
          <span className="flex items-center gap-1 text-[10px] text-white/30"><Clock size={10} /> {readTime(post.wordCount)}</span>
          <span className="flex items-center gap-1 text-[10px] font-semibold text-[#7B5FFF] group-hover:gap-2 transition-all">Read <ChevronRight size={11} /></span>
        </div>
      </motion.div>
    </Link>
  );
...
function EmptyState({ search, category, total }: { search: string; category: string; total: number }) {
  if (total === 0) {
    return (
      <div className="text-center py-32 px-4">
        <div className="w-20 h-20 rounded-2xl flex items-center justify-center mx-auto mb-6"
          style={{ background: 'linear-gradient(135deg,rgba(0,196,255,0.1),rgba(123,95,255,0.1))' }}>
        ...
        <p className="text-white/40 max-w-md mx-auto mb-6 text-sm leading-relaxed">
          Our team is crafting in-depth guides. Check back soon, or sign up to get notified.
        </p>
        <Link to="/auth" className="inline-flex items-center gap-2 px-6 py-3 rounded-xl text-sm font-bold text-white"
          style={{ background: 'linear-gradient(135deg,#7B5FFF,#D63DF6)' }}>
  ...
  return (
    <div className="text-center py-20">
      <p className="text-white/40 text-sm">No articles found for "{search || category}".</p>
    </div>
  );
}
...
function injectArticleSchema(post: any) {
  const schema = {
    ...
    publisher: { '@type': 'Organization', name: 'MIXXEA', logo: { '@type': 'ImageObject', url: `${SITE_URL}/logo.png` } },
...
export function BlogPost() {
  ...
  useSEO({
    ...
    description: post?.metaDescription || 'Expert guides for independent artists and labels.',
  ...
  useEffect(() => {
    ...
    api.blogGetPost(slug)
      .then((res: any) => { setPost(res.post); injectArticleSchema(res.post); })
      ...
      <div className="min-h-screen bg-black flex items-center justify-center px-5">
        <div className="text-center max-w-lg">
          ...
          <p className="text-white/40 text-sm mb-6">
            The article <code className="text-[#7B5FFF]">/blog/{slug}</code> doesn't exist yet.
          </p>
      ...
      <main className="pt-28 pb-24 px-5">
        <div className="max-w-3xl mx-auto">
          ...
          <div className="flex items-center gap-3 mb-5">
            ...
            <span className="flex items-center gap-1.5 text-xs text-white/30"><Clock size={11} /> {readTime(post.wordCount)}</span>
            <span className="text-xs text-white/30">{fmtDate(post.publishedAt)}</span>
          </div>

          <h1 className="text-3xl sm:text-5xl font-black text-white leading-tight mb-5">{post.title}</h1>
          ...
          <div className="h-px w-full mb-10" style={{ background: 'linear-gradient(90deg,#7B5FFF30,#D63DF620,transparent)' }} />
            ...
            <div className="mb-12">
              ...
              <div className="space-y-3">
                {post.faqSchema.map((faq: any, i: number) => (
                  <div key={i} className="border border-white/[0.07] rounded-2xl overflow-hidden">
                    <button
                      onClick={() => setOpenFaq(openFaq === i ? null : i)}
                      className="w-full flex items-center justify-between px-5 py-4 text-left hover:bg-white/[0.02] transition-colors"
                    >
                      <span className="text-sm font-semibold text-white pr-4">{faq.question}</span>
                      <ChevronRight size={14} className={`flex-shrink-0 text-[#7B5FFF] transition-transform ${openFaq === i ? 'rotate-90' : ''}`} />
                    </button>
                    {openFaq === i && (
                      <div className="px-5 pb-4 text-sm text-white/50 leading-relaxed">{faq.answer}</div>
                    )}
                  </div>
                ))}
              </div>
          ...
          {post.internalLinks?.length > 0 && (
            <div className="mb-12">
              <h3 className="text-sm font-bold text-white/40 uppercase tracking-widest mb-4">Related Reading</h3>
          ...
          >
            ...
            <p className="text-white/50 text-sm mb-6 max-w-md mx-auto">
              Distribute to 70+ platforms, launch Spotify and TikTok campaigns, pitch playlists, and manage your publishing — all from one dashboard.
            </p>
